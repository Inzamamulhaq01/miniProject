...
<body>
  ...
  <script>
    ...
    async function showSummary() {
        ...
        params.set('outstandingBalance', parseFloat(data.principal));
        ...
        const chartUrl = `chart.html?${params.toString()}`;
    ...
    //     // Function to fetch loan data and calculate outstanding balance
    // async function getOutstandingBalance(userId) {
    //   const loanData = await fetchLoanData(userId);
    //   if (loanData && loanData.success) {
    //     const outstandingBalance = calculateOutstandingBalance(loanData);
    //     console.log("Outstanding Balance: ₹", outstandingBalance);
    //     return outstandingBalance;
    //   } else {
    //     console.log('Failed to fetch loan data or no data found.');
    //   }
    // }

    const userId = <?php echo json_encode($user_id); ?>;
    // console.log(fetchLoanData(userId));
    // console.log(fetchLoanData(userID));
  ...
  // redirectToChart();
    ...
    // function calculateOutstandingBalance(loanData) {
    //   const principal = parseFloat(loanData.data.principal); // Total loan amount
    //   const principalPaid = parseFloat(loanData.data.principal_paid); // Total principal paid so far
    //   const interestPaid = parseFloat(loanData.data.interest_paid); // Total interest paid so far
    //   const totalPaid = parseFloat(loanData.data.total_paid); // Total paid so far (principal + interest)
    //   const emi = parseFloat(loanData.data.emi); // Monthly EMI
    //   const monthsElapsed = loanData.data.months_elapsed; // Number of months that have passed
    //   const repaymentStatus = loanData.data.repayment; // Repayment status ("yes" or "no")
      
      
    //   // Remaining principal is calculated based on what has been repaid
    //   const remainingPrincipal = principal - principalPaid;
    
    //   return remainingPrincipal.toFixed(2); // Return remaining principal
    // }
